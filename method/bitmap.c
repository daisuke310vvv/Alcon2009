...
Image* Histogram(Image *img)
{
	unsigned long hist[256], max=0;
	int i, j;
	Image *temp;
	...
	if((temp = Create_Image(256, 256)) == NULL){
	...
	for(i=0; i<256; i++){
		for(j=0; j<(int)(256*hist[i]/(double)max); j++){
			temp->data[(256-j)*256 + i].r = temp->data[(256-j)*256 + i].g = temp->data[(256-j)*256 + i].b = 255;
		}
	}
